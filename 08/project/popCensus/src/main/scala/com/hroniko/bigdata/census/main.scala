...
object main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val fullTextFilePreCensus = textFilePreCensus.filter(line => ! isZero(line));
    ...
    val preCensusRecord = fullTextFilePreCensus
      .map(line => parse(line)) // Превращаем в объекты
    ...
    val fullTextFileFinalCensus = textFileFinalCensus.filter(line => ! isZero(line));
    ...
    val  tuplePreCensusRecord = preCensusRecord.map(ocr => OCRtoKeyValue(ocr))
    // 9.2 Для финальных данных
    val  tupleFinalCensusRecord = finalCensusRecord.map(ocr => OCRtoKeyValue(ocr))
    ...
    val diffCensusRecord = tupleCensusRecordAfterJoin
      .sortByKey()
      .mapValues(tupleOCR => OCRdistinction(tupleOCR))

    // 13 Конвертируем значения к строке
    val diffString = diffCensusRecord.values.map(ocr => OCRtoString(ocr))
    ...
    val procentazhRegionRDD = groupRegionRDD
      .sortByKey()
      .mapValues(value => calcProcent(value.toArray))
    ...
    val textFilePermskyKraiWihoutHeader = textFilePermskyKrai.filter(line => ! isHeader(line));
    ...
    val permskyKraiKeyValue = textFilePermskyKraiWihoutHeader
      .map(line => parsePermskiKrai(line)) // Превращаем в RDD типа Ключ-Значение (Ключ - дата записи, Значение - объект Запись)
    ...
    val procentazhPermskiKraiRDD = groupPermskiKraiOCR
      .sortByKey()
      .mapValues(value => calcProcentPermskiKrai(value.toArray))
    ...
    val groupPriristRDD = groupPririst.mapValues(value => calcProcentPermskiKrai(value.toArray))
